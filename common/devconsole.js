...
function levelAllowed(level, type){
  const idx = LEVEL_ORDER.indexOf(level || "info");
  return LEVEL_ORDER.indexOf(type) >= idx;
}

export const DevConsole = {
  init({ mount, settingsGet, settingsSet }){
    const state = {
      paused:false,
      entries:[],
    ...
    tabs.forEach(t => t.addEventListener("click", () => {
      ...
      Object.entries(panels).forEach(([k,el]) => {
        ...
        if (k === "payload") refs.json.classList.toggle("pw-dc__json--show", show);
      });
    }));
    ...
    root.querySelector('[data-action="export"]').addEventListener("click", () => {
      const blob = new Blob([JSON.stringify({
        meta:{ app:"PersonaWorks Stage", version:"1.0.0", ts:Date.now() },
        settings:{ level: state?.cfg?.level, maxEntries: state?.cfg?.maxEntries },
        logs: state.entries
      }, null, 2)], { type:"application/json" });
      const a = document.createElement("a"); a.href = URL.createObjectURL(blob);
      a.download = `devconsole_export_${Date.now()}.json`; a.click(); URL.revokeObjectURL(a.href);
    });
    ...
    function addEntry(evt){
      ...
      if (!levelAllowed(state?.cfg?.level, type)) return;
      const entry = {
      ...
      renderRow(entry);
      refs.count.textContent = String(state.entries.length);
    }
    ...
    function render(){
      refs.list.innerHTML = "";
      state.entries.filter(passFilters).forEach(renderRow);
      refs.count.textContent = String(state.entries.length);
      refs.json.textContent = state.selected ? JSON.stringify(state.selected, null, 2) : "";
      refs.tokIn.textContent = String(state.totals.in);
      refs.tokOut.textContent = String(state.totals.out);
    ...
    function renderRow(e){
      ...
      row.className = "pw-dc__row" + (e.type === "error" ? " pw-dc__row--err" : "");
      row.innerHTML = `<div class="pw-dc__cell">${e.time}</div>
        <div class="pw-dc__cell">${e.scope}</div>
        <div class="pw-dc__cell">[${e.type}] ${e.message}${e.durationMs?` • ${e.durationMs}ms`:""}</div>`;
      row.addEventListener("click", () => { state.selected = e; refs.json.textContent = JSON.stringify(e, null, 2);
